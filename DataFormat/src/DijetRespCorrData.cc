...
Int_t DijetRespCorrData::GetMinUnfixedParameter(void) const
{
  return fMinUnfixedParameter;
}

Int_t DijetRespCorrData::GetMaxUnfixedParameter(void) const
{
  return fMaxUnfixedParameter;
}

void DijetRespCorrData::GetArray(TArrayD& array) const
{
  array = fArray;
  return;
}

void DijetRespCorrData::SetUnfixedParameters(Int_t ietaMin, Int_t ietaMax)
{
  fMinUnfixedParameter = ietaMin;
  fMaxUnfixedParameter = ietaMax;
  return;
}

void DijetRespCorrData::SetArray(TArrayD array)
{
  fArray = array;
  return;
}
...
void DijetRespCorrData::doFit(TArrayD& respcorr, TArrayD& respcorre)
{
  ...
  for(int i=0; i<respCorrInit.GetSize(); i++) {
    ...
    if(fabs(ieta) < GetMinUnfixedParameter() || fabs(ieta) > GetMaxUnfixedParameter()) gMinuit->FixParameter(i);
